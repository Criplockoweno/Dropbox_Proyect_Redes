...
class HTTPRequestHandler():
    ...
    def handle_file_request(self, request, headers):
        boundaries = '--' + re.search(r'boundary=([^;\\]+)', headers['Content-Type']).group(1)
        ...
        filename = re.search(r'filename=([^;\\]+)', body_headers['Content-Disposition']).group(1).replace('"', '')
    ...
    def parse_request(self, request):
        ...
        if 'Content-Length' in headers:
            request = self.handle_chunked_request(request, int(headers['Content-Length']))
